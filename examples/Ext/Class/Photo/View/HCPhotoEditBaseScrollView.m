...
-(instancetype)initWithFrame:(CGRect)frame  bottomTitle:(NSString*)title  imagesArray:(NSArray*)images  titlesArray:(NSArray*)titles buttonWidth:(float)width  imageSize:(float)size
{
    ...
    if (self) {
        ...
        [cancelBtn setImage:image_named(@"photo_bottom_bar_cancel") forState:UIControlStateNormal];
        [cancelBtn setImage:image_named(@"photo_bottom_bar_cancel_light") forState:UIControlStateNormal];
        ...
        [finishBtn setImage:image_named(@"photo_bottom_bar_ok") forState:UIControlStateNormal];
        [finishBtn setImage:image_named(@"photo_bottom_bar_ok_light") forState:UIControlStateNormal];
...
//                NSString *title = (index < titles.count) ? titles[index]: nil;
                UIImage *hightImage = image_named(name2);
                if (!hightImage) {
                    hightImage = image_named([name stringByAppendingString:@"_Light"]);
                }
                HCPhotoEditCustomButton *btn = [[HCPhotoEditCustomButton alloc] initWithImage:image_named(name) highlightedImage:hightImage title:titles[index] font:12 imageSize:size];
                btn.tag = index + 10;
