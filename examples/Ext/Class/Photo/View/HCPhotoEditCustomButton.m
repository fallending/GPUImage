...
-(instancetype)initWithImage:(UIImage*)image  highlightedImage:(UIImage*)hightImage title:(NSString*)title  font:(float)font imageSize:(float)size
{
    ...
    if (self) {
        ...
        [self setBackgroundColor:color_with_rgb(25, 25, 25)];
        self.titleLabel.textAlignment = NSTextAlignmentCenter;
        self.titleLabel.font = [UIFont systemFontOfSize:14];
        [self setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [self setTitleColor:color_with_rgb(250, 223, 84) forState:UIControlStateHighlighted];
        [self setTitleColor:color_with_rgb(250, 223, 84) forState:UIControlStateSelected];
...
-(void)setHighlighted:(BOOL)highlighted
{
    ...
    if (highlighted) {
        [self setBackgroundColor:color_with_rgb(0, 0, 0)];
        if (_hightImage) {
            [self setImage:_hightImage forState:UIControlStateNormal];
        }
        
        [self setTitleColor:color_with_rgb(250, 223, 84) forState:UIControlStateNormal];
    }else{
